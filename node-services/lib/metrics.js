...
class GCMetrics {
  ...
  constructor() {
    this.DEFINITIONS = config.metrics.definitions;
    ...
    if( config.google.serviceAccountFile ) {
      ...
      this.setGlobalLabels({
        instance: config.server.url,
        server_env: config.server.env,
   ...
   * @param {String} id id for metric.  Note, this is only for logging, metrics will not contain the url 
   * @param {Number} value metric value to record 
   * @param {Object} opts 
   * @param {Error} opts.error optional error object for logging
   * @returns {Promise}
   */
  logIndexEvent(type, labels, value, id, opts={}) {
    ...
    if( labels?.status === 'error' ) {
      ...
      logger.error.apply(logger, [type, labels, id, error]);
    } else if( opts.log !== false ) {
      logger.info.apply(logger, [type, labels, id, value]);
    }
